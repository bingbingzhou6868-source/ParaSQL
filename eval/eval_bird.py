...
PROCESSED_DIR = Path("data/processed/bird")


def get_db_path(db_id: str, split: str) -> str:
    return str(BIRD_DIR / split / f"{split}_databases" / db_id / f"{db_id}.sqlite")
...
def run_eval(args):
    ...
    print(f"评测 BIRD {args.split}: {len(data)} 条")

    predictions, gold_sqls, db_paths = [], [], []
    ...
    for i in tqdm(range(0, len(data), args.batch_size), desc="推理中"):
        ...
        db_paths.extend([get_db_path(item["db_id"], args.split) for item in batch])
    ...
    diff_stats = {}
    for diff, ex_r in zip(difficulties, ex_results):
    ...
    print("\n按难度分组:")
    for diff, stat in sorted(diff_stats.items()):
        acc = stat["correct"] / stat["total"] if stat["total"] > 0 else 0
        print(f"  {diff:12s}: {acc:.4f} ({stat['correct']}/{stat['total']})")
    ...
    out_file = output_dir / f"bird_{args.split}_results.json"
